...
export function generateSKUDetails(
  ...
  const sauceName = isFranchisePeriod && (isPlatformChannel || !hasSelfSauce) ? '官方拌料' : hasSelfSauce ? '自研拌料' : '官方拌料'
  ...
  if (comboACount > 0) {
    details.push({
      ...
      ingredients: [
        ...
        { name: sauceName, count: comboACount },
  ...
  if (comboBCount > 0) {
    details.push({
      ...
      ingredients: [
        ...
        { name: sauceName, count: comboBCount },
...
export function generateChannelOrderForecasts(
  ...
  for (const channel of CHANNEL_PRIORITY) {
    ...
    const skuBreakdown = calculateIngredientsForOrders(forecastOrders, hasBeerLicense, isFranchisePeriod, enabledSkus, hasActiveEvent, hasSelfSauce, isPlatform)
    const skuProducts = generateSKUProducts(forecastOrders)
    const skuDetails = generateSKUDetails(forecastOrders, hasBeerLicense, isFranchisePeriod, isPlatform, hasSelfSauce)
...
export function deductIngredientsFromPlan(
  ...
  for (const [channel, orderCount] of fulfilledOrders) {
    const isPlatform = channel === 'mt' || channel === 'sg' || channel === 'jd'
    const needed = calculateIngredientsForOrders(orderCount, hasBeerLicense, useOfficialSauce, enabledSkus, hasActiveEvent, hasSelfSauce, isPlatform)
