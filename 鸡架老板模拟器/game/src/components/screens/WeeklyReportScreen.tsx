...
export default function WeeklyReportScreen() {
  ...
  return (
    <div>
      ...
      {state.leftFranchise && !state.acquiredBrand && (
        <div className="card">
          <div className="card-title" style={{ color: 'var(--color-gold)' }}>👑 收购总部</div>
          <div style={{ fontFamily: 'var(--pixel-font)', fontSize: '12px', marginBottom: '12px' }}>
            收购品牌总部，全渠道单量上涨10倍，品牌商誉+800,000元
            <br />
            收购费用：500,000元 | 当前现金：{fmtMoney(cash)}元
            {state.acquiredBrandWeek && (
              <><br />收购后已过{state.absoluteWeek - state.acquiredBrandWeek}周（3个月后结算结局）</>
            )}
          </div>
          <Button
            variant="primary"
            ...
            onClick={() => dispatch({ type: 'ACQUIRE_BRAND' })}
          >
            收购总部（50万）
          </Button>
        </div>
      ...
      {state.acquiredBrand && (
        <div className="card">
          <div className="card-title" style={{ color: 'var(--color-gold)' }}>👑 已收购总部</div>
          <div style={{ fontFamily: 'var(--pixel-font)', fontSize: '12px' }}>
            全渠道单量×10 | 品牌商誉：{fmtMoney(state.brandGoodwill)}元
            <br />
            收购后已过{state.acquiredBrandWeek ? state.absoluteWeek - state.acquiredBrandWeek : 0}周
            {state.acquiredBrandWeek && state.absoluteWeek - state.acquiredBrandWeek < 12 && (
              <span style={{ color: '#e67e22' }}>（还需{12 - (state.absoluteWeek - state.acquiredBrandWeek)}周结算结局）</span>
            )}
          </div>
        </div>
      )}
