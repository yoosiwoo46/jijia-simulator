                    ...
                    {forecast.isOutsourced ? (
                      <>
                        <span style={{ fontFamily: 'var(--pixel-font)', fontSize: '11px', color: '#e67e22' }}>
                          🔄 已{forecast.outsourceType === 'emergency' ? '紧急' : '长期'}外包（费用=收入×{forecast.outsourceType === 'emergency' ? '95' : '90'}%）
                          {forecast.outsourceType === 'longterm' && (forecast.outsourceWeeksLeft ?? 0) > 0 && (
                            <span style={{ color: '#999', marginLeft: '4px' }}>（剩余{forecast.outsourceWeeksLeft}周不可终止）</span>
                          )}
                        </span>
                        <Button
                          variant="danger"
                          size="sm"
                          disabled={forecast.outsourceType === 'longterm' && (forecast.outsourceWeeksLeft ?? 0) > 0}
                          onClick={() => dispatch({ type: 'STOP_OUTSOURCE', payload: { channel: forecast.channel } })}
                        >
                          终止外包
                        </Button>
                      </>
                      ...
                      <>
                        ...
                        <Button
                          variant="secondary"
                          size="sm"
                          disabled={forecast.orders < 500}
                          onClick={() => dispatch({ type: 'OUTSOURCE_CHANNEL', payload: { channel: forecast.channel, outsourceType: 'longterm' } })}
                        >
                          长期外包
                        </Button>
                        {forecast.orders < 500 && (
                          <span style={{ fontFamily: 'var(--pixel-font)', fontSize: '10px', color: '#e74c3c' }}>单量不足500</span>
                        )}
                      </>
                    )}
                  </div>
                  <div style={{ fontFamily: 'var(--pixel-font)', fontSize: '10px', color: '#999', marginTop: '4px' }}>
                    外包费用：紧急=收入×95%，长期=收入×90%（长期最短3个月）
                  </div>
                </>
              )}
            </div>
            ...
            <div style={{ marginBottom: '8px' }}>
              ...
              {ingredientShortage.length === 0 ? (
                ...
                <span className="text-bold text-red">
                  如果单量浮动，预计：{ingredientShortage.map(s => `${s.name}缺${s.gap}份`).join('、')}
                </span>
              )}
            </div>
          </div>
