...
export type GameAction =
  ...
  | { type: 'ACQUIRE_BRAND' }
  | { type: 'DISMISS_DIRECT_STORE_STORY' }
...
function gameReducer(state: GameState, action: GameAction): GameState {
  switch (action.type) {
    ...
    case 'STOP_OUTSOURCE': {
      const forecast = state.channelOrderForecasts.find(f => f.channel === action.payload.channel)
      if (forecast?.outsourceType === 'longterm' && (forecast.outsourceWeeksLeft ?? 0) > 0) {
        return {
          ...state,
          notifications: [...state.notifications, '长期外包未满3个月，无法终止'],
        }
      }
      return {
        ...state,
    ...
    case 'ACQUIRE_BRAND': {
      if (state.cash < 500000 || !state.leftFranchise || state.acquiredBrand) return state
      return {
        ...state,
        cash: state.cash - 500000,
        acquiredBrand: true,
        acquiredBrandWeek: state.absoluteWeek,
        brandGoodwill: state.brandGoodwill + 800000,
        pendingAcquireStory: null,
        notifications: [...state.notifications, '🎉 你成功收购了品牌总部！全渠道单量上涨10倍！'],
      }
    }

    case 'DISMISS_DIRECT_STORE_STORY': {
      return {
        ...state,
        pendingDirectStoreStory: null,
        brandAwareness: Math.max(0, state.brandAwareness - 10),
        notifications: [...state.notifications, '总部直营店开业，品牌知名度下降'],
      }
    }
